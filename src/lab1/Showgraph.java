...
public class Showgraph {
	...
	public MatrixDG createDirectedGraph(String filename) // createDirectedGraph createDirectedGraph
	{
		...
		for (int i = 0; i < s2.length() - 1; i++) {
			...
			} else if (b == 33 || b == 34 || b == 39 || b == 40 || b == 41 || b == 44 || b == 45 || b == 46 || b == 58
					|| b == 59 || b == 63) {
				if (f == 0) {
					s3 = s3 + ' ';
					f = 1;
		...
		for (i = 0; i < cw2; i++) {
			...
			if (!(words1[i]).equals("") && !(words1[i + 1]).equals(""))// if (words1[i]!="" && words1[i+1]!="")
			{
	...
	private String calcShortestPath(MatrixDG g, String word1, String word2) {
		...
		for (int j = 99; j >= 0; j--) {
			if (!(path[j]).equals("") && j != 0)// if (path[j]!="" && j!=0)
			{
				s = s + path[j] + " -> ";
